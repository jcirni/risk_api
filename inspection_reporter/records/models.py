...
class Restaurant(BaseModel):
    ...
    state = models.CharField(max_length=2,validators=[validators.valid_state_abbrev])
    #assume 5 digit area code, not +4zip
    postal_code = models.CharField(max_length=5, blank=True, validators=[validators.valid_zip])
    #flag for updating aggregates only on new inspection data
    is_current = models.BooleanField(default=True)
    average_score = models.DecimalField(max_digits=5, decimal_places=2, default=-1.00)
    average_violations = models.DecimalField(max_digits=5,decimal_places=2, default=-1.00)
    total_inspection = models.IntegerField(default=0)
    
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['name', 'street_address'], name='unique_restaurant')
        ]
...
class InspectionViolation(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['violation_id','inspection_id'], name='unique_inspection_violation')
        ]
