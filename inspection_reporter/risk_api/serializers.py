...
class RestaurantSerializer(serializers.Serializer):
    ...
    def update(self, instance, validated_data):
        restaurant_id = validated_data.get('restaurant_id', instance.restaurant_id)
    ...
    class Meta:
        model = Restaurant
        fields = ['restaurant_id', 'name', 'street_address', 'city', 'state', 'postal_code']


class ViolationSerializer(serializers.ModelSerializer):
    class Meta:
        model = Violation
        fields = ['violation_id', 'is_critical', 'code', 'description', 'comments']
...
class InspectionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Inspection
        fields = ['inspection_id','inspection_date','score','comments','violations', 'restaurant']

    def create(self, validated_data):
        violations_data = validated_data.pop('violations')
        restaurant_data = validated_data.pop('restaurant')
        #If new restaurant, create it
        r = get_if_exists(Restaurant, restaurant_id=restaurant_data['restaurant_id'])
        if r is None:
            r = Restaurant(**restaurant_data)
            r.save()
        inspection = Inspection.objects.create(restaurant_id=r.restaurant_id, **validated_data)
        #flip flag
        r.is_current = False
        r.save()
        
        for obj in violations_data:
            violation = Violation.objects.create(inspection_id=inspection.inspection_id, **obj)
            InspectionViolation.objects.create(inspection_id=inspection.inspection_id, violation=violation)
